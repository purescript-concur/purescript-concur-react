-- | Synthethic event representation for Stark.
module Stark.SyntheticEvent
  ( SyntheticEvent
...
type SyntheticEvent_ (r :: # Type) = Record r

data NativeEventTarget = NativeEventTarget

data NativeEvent = NativeEvent

data NativeDataTransfer = NativeDataTransfer

data NativeAbstractView = NativeAbstractView

data NativeTouchList = NativeTouchList
...
isDefaultPrevented _ = pure true

stopPropagation :: forall r. SyntheticEvent_ r -> Effect Unit
stopPropagation _ = pure unit

isPropagationStopped :: forall r. SyntheticEvent_ r -> Effect Boolean
isPropagationStopped _ = pure true

persist :: forall r. SyntheticEvent_ r -> Effect Unit
persist _ = pure unit
